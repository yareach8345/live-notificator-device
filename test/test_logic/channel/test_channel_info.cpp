...
void test_channel_info_gen() {
    ...
    TEST_ASSERT_EQUAL(CLOSE, ci.get_live_state_variant().get_live_state_type());
}
...
void test_channel_info_change_with_ref() {
    ChannelInfo ci(
        ChannelId(CHZZK, "123"),
        ChannelDetail("tester", 123, 12, String("#ffffff")),
        LiveOpen("hello world", "test", 1)
    );

    const long before_user_count = ci.get_live_state_variant().get_live_state_by_open().get_concurrent_user_count();
    ci.get_live_state_variant_ref()->get_live_state_by_open_ref()->set_concurrent_user_count(100);
    const long after_user_count = ci.get_live_state_variant().get_live_state_by_open().get_concurrent_user_count();
...
void test_channel_info_live_state_change() {
    ...
    const LiveStateType live_state_type_before = ci.get_live_state_variant().get_live_state_type();

    ci.set_live_state(LiveOpen("testing", "test", 12));

    const LiveStateType live_state_type_after = ci.get_live_state_variant().get_live_state_type();
