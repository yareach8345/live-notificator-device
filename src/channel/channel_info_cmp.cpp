...
bool ChannelInfoCmp::operator()(ChannelInfo c1, ChannelInfo c2) const {
    ...
    if (
        c1.get_live_state()->get_is_open() && c2.get_live_state()->get_is_open() &&
        dynamic_cast<LiveOpen*>(c1.get_live_state().get())->get_concurrent_user_count() != dynamic_cast<LiveOpen*>(c2.get_live_state().get())->get_concurrent_user_count()
    ) {
